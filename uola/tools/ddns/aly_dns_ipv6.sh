...
# 更新记录值 (RecordID)
update_record() {
	local queryString="RR=$SubDomain&RecordId=$1&SignatureMethod=HMAC-SHA1&SignatureNonce=$timestamp&SignatureVersion=1.0&TTL=$TTL&Timestamp=$timestamp&Type=$Type&Value=$(enc $ipv6)"
	local result=$(send_request "UpdateDomainRecord" $queryString)
}

# 添加记录值 (RecordID)
add_record() {
	local queryString="RR=$SubDomain&SignatureMethod=HMAC-SHA1&SignatureNonce=$timestamp&SignatureVersion=1.0&TTL=$TTL&Timestamp=$timestamp&Type=$Type&Value=$(enc $ipv6)"
